...
def random_PT_dag_list_new(dag_list, sample_num):
    ret_dag_list = [[float('inf'), float('inf'), float('inf'), copy.deepcopy(dag_list)] for _ in range(sample_num)]    # (1) cri 1 (2) makespan (3) dag_list
    for ret_dag_x in ret_dag_list:
        for tdag_x in ret_dag_x[3]:
            for node_x in tdag_x.nodes(data=True):
...
if __name__ == "__main__":
    ...
    # for dag_x in HW_DAG:
    #     print(f'dag{dag_x.graph["DAG_ID"]}_volum:{DFA.get_dag_volume(dag_x)}')
    HW_DAG[5].nodes[1]['WCET'] = 0
    # print(HW_DAG[5].nodes[1])
    dag_id_list = [1, 5]
    ...
    ret_dag_list.sort(key=lambda x: x[0], reverse=False)
    ret_dag_list.sort(key=lambda x: x[2], reverse=False)
    print([ret_dag_x[2] for ret_dag_x in ret_dag_list])
