...
if __name__ == "__main__":
    ...
    for dag_cri, dag_id in enumerate(dag_id_list):
        DFA.dag_data_initial(HW_DAG[dag_id], DAGType=int(dag_cri), DAG_id=int(dag_cri), Period=Period, Critic=dag_cri + 1)  # (2.1) 配置DAG的初始参数
        # DFA.dag_data_initial(HW_DAG[1], DAGType=int(2), DAG_id=int(2), Period=Period, Critic=2)  # (2.1) 配置DAG的初始参数

    All_DAG_list = []
    for dag_id_x in  dag_id_list:
        All_DAG_list += [copy.deepcopy(HW_DAG[dag_id_x]) for _ in range(dag_num)]
    ...
    """
    for dag_id_x in  dag_id_list:
        for node_x in HW_DAG[dag_id_x].nodes(data=True):
            node_x[1]['PT'] = node_x[1]['Prio']
    """
