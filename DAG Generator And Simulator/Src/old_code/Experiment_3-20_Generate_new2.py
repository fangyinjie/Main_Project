...
def Exam_function(param):
    ...
    dag_id = param['dag_id']
    exam_address = './NEW_RET/flow-{0}/jiter-{1}/{2}/'.format(flow_num, jiter_rate, dag_id)
...
class myThread(threading.Thread):
    ...
    def run(self):
        for e_counter in range(self.counter):
            ...
            Total_DAG_list = DG.Algorithm_input('FLOW', Param_dict)     # dag生成；
            ...
            for dag_id, dag_x in enumerate(Total_DAG_list):
                ...
                exam_address = './NEW_RET_2/flow-{0}/jiter-{1}/{2}/'.format(flow_num, jiter_rate, dag_id)
...
if __name__ == "__main__":
    ...
    Total_Time = 3 * TTL
    # (2) DAG generation
    root_addr = 'D:/github/DAG_Scheduling_Summary/Exam_Input_data/xlsx_data/wireless/DAG_Data_flow_new_m2.xlsx'
    ...
    for jiter_rate in [0.1]:
        ...
        for flow_num in range(2, 3):
            param_l = [copy.deepcopy(All_flow_list), {'jiter_rate': jiter_rate, 'flow_num': flow_num, 'DAG_Num': 1}]
    ...
    df.to_csv('./ret_data{0}.csv'.format(time.time()))
