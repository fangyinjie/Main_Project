...
def __rta_basics_np_single(DAGx:nx.DiGraph,
                           Core_Num: int):
    node_list = list(DAGx.nodes())
    paths = list(nx.all_simple_paths(DAGx, node_list[0], node_list[-1]))
    ...
    for x in paths:
        ...
        temp_path_weight = 0
        for y in x:
            temp_all_node = DAGx.nodes(data=True)
            temp_ance = list(nx.ancestors(DAGx, y))
            temp_desc = list(nx.descendants(DAGx, y))
            temp_self = x
            sub_node = DAGx.nodes[y]
