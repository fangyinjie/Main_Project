...
if __name__ == "__main__":
    ...
    for core_num in [40]:
        ...
        Dispatcher.run()
        ret_h_1 = Dispatcher.Core_Data_List
        print(f'type4_1_MAKESPAN_c1:{Core.ret_dag_cri_makespan(ret_h_1, 1)}', end=',')
        print(f'type4_1_MAKESPAN_c2:{Core.ret_dag_cri_makespan(ret_h_1, 2)}', end=',')
        print(f'type4_1_MAKESPAN:{Core.ret_makespan(ret_h_1)}', end=',')
        print(f'type4_1_C1_surpass:{100 * (Core.ret_dag_cri_makespan(ret_h_1, 1) - min_makespan) / min_makespan}', end=',')
        print(f'type4_1_m_improve:{100 * (p_m - Core.ret_makespan(ret_h_1)) / p_m}', end=',')
        ...
        Dispatcher = SS.Dispatcher_Workspace([copy.deepcopy(ret_dag_x[3]),{'Core_Num': core_num, 'Total_Time': Period * cycle * 2,
                           'Enqueue_rank': False, 'Priority_rank': True, 'Preempt_type': 'type4_2', 'Dynamic': False}])
        Dispatcher.run()
        ret_h_2 = Dispatcher.Core_Data_List
        print(f'type4_2_MAKESPAN_c1:{Core.ret_dag_cri_makespan(ret_h_2, 1)}', end=',')
        print(f'type4_2_MAKESPAN_c2:{Core.ret_dag_cri_makespan(ret_h_2, 2)}', end=',')
        print(f'type4_2_MAKESPAN:{Core.ret_makespan(ret_h_2)}', end=',')
        print(f'type4_2_C1_surpass:{100 * (Core.ret_dag_cri_makespan(ret_h_2, 1) - min_makespan) / min_makespan}', end=',')
        print(f'type4_2_m_improve:{100 * (p_m - Core.ret_makespan(ret_h_2)) / p_m}')


        SRS.show_core_data_list({'P':SELF_P_h},
                                'Show', '',
                                # Data_Output_Addr,'Save',
                                copy.deepcopy([HW_DAG[1], HW_DAG[5]]), Period, cycle)
        SRS.show_core_data_list({'S':ret_h_1},
                                'Show', '',
                                # Data_Output_Addr,'Save',
                                copy.deepcopy([HW_DAG[1], HW_DAG[5]]), Period, cycle)
        SRS.show_core_data_list({'M':ret_h_2},
                                'Show', '',
                                # Data_Output_Addr,'Save',
                                copy.deepcopy([HW_DAG[1], HW_DAG[5]]), Period, cycle)