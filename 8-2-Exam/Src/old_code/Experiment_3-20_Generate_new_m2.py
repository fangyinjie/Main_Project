...
def Exam_function(param):
    ...
    dag_id = param['dag_id']
    exam_address = './NEW_RET_M2/flow-{0}_jiter-{1}_{2}/'.format(flow_num, jiter_rate, dag_id)
...
class MyThread_2(threading.Thread):  # 重写threading.Thread类，加入获取返回值的函数
    def __init__(self, url):
        threading.Thread.__init__(self)     # (1) 重构run函数必须写
        # super(MyThread_2, self).__init__()  # (2) 重构run函数必须写
        self.url = url
    def run(self):
